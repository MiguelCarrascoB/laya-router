...
from .laya_backend import ROUTE_QUESTIONS, LayaBackend, build_backend
...
class ModelRouter:
    def __init__(self, backend: LayaBackend | None = None, confidence_threshold: float | None = None) -> None:
        ...
        self.guardrail = Guardrail(self.backend)
    ...
    def triage(self, task: str, context: str = "") -> dict[str, Any]:
        # The guardrail module owns triage; the router delegates so both
        # features share one backend while keeping a module boundary.
        return self.guardrail.triage(task, context)
